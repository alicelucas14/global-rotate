...
export async function checkIndonesia(url) {
  try {
    ...
    // If no nodes responded at all, check was inconclusive
    if (responded === 0) {
      return { available: null, idNodesUp: 0, idNodesTotal: idNodes.length, idNodesResponded: 0 };
    }
    ...
    return {
      ...
      idNodesResponded: responded
    };
  } catch (err) {
    return { available: null, idNodesUp: 0, idNodesTotal: 0, idNodesResponded: 0, error: String(err && err.message || err) };
  }
}
...
export async function evaluate(url) {
  ...
  if (!direct.ok) {
    ...
    status = 'ACTIVE';
  } else if (id.available === false && id.idNodesResponded >= 2) {
    // Only mark BLOCKED if at least 2 Indonesian check-host nodes explicitly responded and failed
    status = 'BLOCKED';
  } else {
    // Indonesia check inconclusive (rate-limited / node errors); trust direct reachability
