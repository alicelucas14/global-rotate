/**
 * Tornado 3D Vortex Background — OriginKit Engine
 * Standalone Three.js implementation
 */
(function() {
  "use strict";
  ...
  function createVortex(canvas, container, cfgRef) {
    if (!window.THREE) {
      console.error("[Vortex] Three.js not loaded.");
      return null;
    }
  ...
  window.initTornado = function(canvasId, containerId, customProps) {
    ...
    const line = { ...DEFAULTS.lineOptions, ...preset.lineOptions };
    const dot = { ...DEFAULTS.dotOptions, ...preset.dotOptions };
    const comet = { ...DEFAULTS.cometOptions, ...preset.cometOptions };
    const shove = { ...DEFAULTS.repelOptions, ...preset.repelOptions };

    const config = {
      floorRadius: preset.bottomRadius / PX_PER_WORLD,
      waistRadius: preset.waistRadius / PX_PER_WORLD,
      crownRadius: preset.topRadius / PX_PER_WORLD,
      waistAt: 1 - preset.waistPosition / 100,
      twist: preset.twist,
      zoom: preset.zoom,
      flowDir: preset.direction === "left" ? -1 : 1,
      flowSpeed: (preset.speed / 100) * (preset.direction === "left" ? -1 : 1),
      lineCount: line.count,
      lineColor: line.color,
      lineGlow: (line.glow / 10) * LINE_GLOW_MAX,
      showDots: preset.dots,
      dotCount: dot.count,
      dotSize: dot.size / DOT_SIZE_SCALE,
      dotColor: dot.color,
      dotGlow: (dot.glow / 10) * DOT_GLOW_MAX,
      dotFlicker: dot.flicker / 10,
      showComets: preset.comets,
    ...
    return createVortex(canvas, container, cfgRef);
  };
})();
